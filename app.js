...
const zoneLabels = {
  ...
  'Asia/Tokyo': 'Tokyo', 'Australia/Sydney': 'Sydney', 'Pacific/Auckland': 'Auckland', 'Asia/Colombo': 'Colombo', 'UTC': 'UTC'
};
...
const cityName = (zone) => {
  if (zoneLabels[zone]) return zoneLabels[zone];
...
function renderClocks(now = new Date()) {
  ...
  $('clock-grid').innerHTML = savedZones.map((clock, index) => `<article class="clock-card" data-index="${index}" draggable="true"><div class="card-top"><span class="card-label">${index === 0 ? 'SAVED LOCATION' : 'WORLD CLOCK'}</span><div class="card-actions"><button class="move-menu-button" data-index="${index}" aria-label="Move ${displayName(clock)}" aria-expanded="false">↕</button><button class="edit-button" data-index="${index}" aria-label="Edit ${displayName(clock)}">✎</button><button class="remove-button" data-index="${index}" aria-label="Remove ${displayName(clock)}">×</button><div class="move-menu" hidden><button data-index="${index}" data-direction="up">Move up</button><button data-index="${index}" data-direction="down">Move down</button></div></div></div><div class="card-name">${displayName(clock)}</div><div class="card-time" data-clock-time="${index}">${format(now, clock.zone, { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: hour12() })}</div><div class="card-bottom"><span class="card-date" data-clock-date="${index}">${format(now, clock.zone, { weekday: 'short', month: 'short', day: 'numeric' })}</span><span class="offset" data-clock-offset="${index}">${offsetText(now, clock.zone)}</span></div></article>`).join('');
...
function updateClockValues(now = new Date()) {
  savedZones.forEach((clock, index) => {
    const timeElement = document.querySelector(
      `[data-clock-time="${index}"]`
    );

    const dateElement = document.querySelector(
      `[data-clock-date="${index}"]`
    );

    const offsetElement = document.querySelector(
      `[data-clock-offset="${index}"]`
    );

    if (timeElement) {
      timeElement.textContent = format(now, clock.zone, {
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit',
        hour12: hour12()
    ...
    if (dateElement) {
      dateElement.textContent = format(now, clock.zone, {
        weekday: 'short',
        month: 'short',
        day: 'numeric'
      });
    ...
    if (offsetElement) {
      offsetElement.textContent = offsetText(now, clock.zone);
    }
  });
}
...
function openCityDialog(index = null) {
  editingIndex = index;
  const clock =
    index === null
      ? { zone: $("zone-select").value, label: "" }
      : savedZones[index];
  $("dialog-title").textContent = index === null ? "Add a city" : "Edit city";
  $("city-zone").value = clock.zone;
  $("city-search").value = zoneInputValue(clock.zone);
  $("city-label").value = clock.label;
  $("city-dialog").showModal();
}
function saveCity() {
  syncZoneInput('city-search', 'city-options', 'city-zone');
  const typedZone = $('city-search').value.trim().toLowerCase();
  const selectedZone = $('city-zone').value;
  if (!selectedZone || zoneInputValue(selectedZone).toLowerCase() !== typedZone) {
    showToast('Select a city from the suggestions');
    $('city-search').focus();
    return false;
  }
  const clock = {
    zone: selectedZone,
    label: $("city-label").value.trim(),
  };
  const duplicate = savedZones.some(
    (item, index) => item.zone === clock.zone && index !== editingIndex,
  );
  if (duplicate) {
    showToast(`${cityName(clock.zone)} is already saved`);
    return false;
  }
  if (editingIndex === null) savedZones.push(clock);
  else savedZones[editingIndex] = clock;
  localStorage.setItem(savedKey, JSON.stringify(savedZones));
  renderClocks();
  showToast(`${displayName(clock)} saved`);
  return true;
}
function removeZone(index) {
  const name = displayName(savedZones[index]);
...
function moveZone(index, direction) {
  ...
  if (next < 0 || next >= savedZones.length) return;
  [savedZones[index], savedZones[next]] = [savedZones[next], savedZones[index]];
  localStorage.setItem(savedKey, JSON.stringify(savedZones));
  renderClocks();
}
...
function applyPreferences() {
  ...
  $("theme-icon").textContent = preferences.theme === "dark" ? "☀" : "☾";
  ...
  document
    ...
    .forEach((button) =>
      button.classList.toggle(
        "active",
        button.dataset.format === preferences.format,
      ),
    );
}
...
populateZones(); applyPreferences(); referenceNow(); renderClocks(displayReferenceDate()); window.setInterval(referenceNow, 1000);
